...
nCorrect = tf.equal(tf.argmax(output_ron_2, 1), desired)
...
def trainNetwork(trainingSteps):
    with tf.Session() as session:
        ...
        for step in range(trainingSteps):
            ...
            print(labels)
            acc, _ = session.run([accuracy, trainingStep], feed_dict= {x: images, desired: labels})
        ...
        images, labels = data.getValidationData()
